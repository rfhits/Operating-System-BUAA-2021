...
俗话说，前人栽树，后人乘凉，现在看了，并非如此，这次上机，感觉OS年年变难。
...
通过`$`判断要输出结构体，然后获取`type`的代码我就不写了，这个比较简单。
...
调试比较简单，在`init`目录下的`init.c`文件中，声明struct，初始化后输出就行。
...
学习struct的初识化，有助于我们修改init.c，测试自己的程序，甚至猜一猜自己的程序错在哪里，调试是不存在的。
...
诡异的是，这样写，可以编译通过，在虚拟机上跑，会输出一些奇怪的数，这也是我在本地测试时放下的第二个错误。

### byebye

大家可以看下17级学长的exam，可能会让大家修改print，支持数组的输出。
